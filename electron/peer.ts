/**
 * electron/peer.ts — PeerJS connection lifecycle manager for Lock Eyes
 *
 * Role:
 ...
 *   by the RENDERER process (not the main process) because PeerJS needs a
 *   MediaStream from getUserMedia, which only works in the renderer.
 *
 ...
 *
 * Architecture note:
 *   This module is externalized from the Vite bundle (see vite.config.ts
 *   rollupOptions.external: ['peerjs']). It's loaded at runtime via
 *   vite-plugin-electron-renderer which allows the renderer to import Node
 *   modules. The `peerjs` package is also externalized and loaded at runtime.
 */
